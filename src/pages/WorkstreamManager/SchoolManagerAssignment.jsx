...
import { UserPlus, Search, Shield, School, Mail, Trash2, Edit } from 'lucide-react';
...
const SchoolManagerAssignment = () => {
    ...
    const handleAssignManager = (e) => {
        ...
        if (editId) {
            ...
            const updatedManagers = managers.map(manager => 
                manager.id === editId
                ? { ...manager, name: newManager.name, email: newManager.email, school: schoolName, status: newManager.schoolId ? 'Assigned' : 'Available' }
                : manager
            );
    ...
    const handleEditManager = (manager) => {
        // Find school ID based on name if possible
        const schoolObj = schools.find(s => s.name === manager.school);
        setNewManager({ 
            name: manager.name, 
            email: manager.email, 
        ...
        <div className="workstream-dashboard">
            ...
            {showAssignForm && (
                <div className="management-card" style={{ marginBottom: '2rem', padding: '2rem' }}>
                    <h3 className="chart-title" style={{ marginBottom: '1rem' }}>{editId ? 'Edit Manager' : 'Appoint New School Manager'}</h3>
                    <form onSubmit={handleAssignManager} style={{ display: 'grid', gap: '1rem', maxWidth: '500px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                            <button type="submit" className="btn-primary">{editId ? 'Update Assignment' : 'Create & Assign'}</button>
                            <button
                                type="button"
                                onClick={() => {
                                    setShowAssignForm(false);
