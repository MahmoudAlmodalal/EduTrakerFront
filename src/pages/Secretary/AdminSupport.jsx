...
const AdminSupport = () => {
    ...
    const handleUserSearch = () => {
        const user = guardians.find(g => g.name.toLowerCase().includes(resetUser.toLowerCase()));
    ...
    const handleStudentSearch = (e) => {
        ...
        const student = students.find(s => s.name.toLowerCase().includes(term.toLowerCase()));
            ...
            <div className="secretary-widgets-grid">
                ...
                <div className="widget-card">
                    ...
                    <div className="form-group">
                        ...
                        {foundStudent && (
                            <div className="text-sm text-green-600 mt-1 flex items-center">
                                <CheckCircle size={14} className="mr-1"/> Found: {foundStudent.name}
                            </div>
                        )}
                    </div>
                    <div className="form-group">
                    ...
                    <div className="flex gap-2 mt-4">
                        <button className="btn-primary flex-1 justify-center" disabled={!foundStudent}>
                    ...
                    <div className="form-group">
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                value={resetUser}
                                onChange={(e) => { setResetUser(e.target.value); setFoundUser(null); }}
                            />
                            <button className="btn-secondary" onClick={handleUserSearch} style={{ padding: '0.5rem', border: '1px solid #d1d5db', background: '#f9fafb' }}>
                                <Search size={18} />
                            </button>
                        </div>
                    </div>
                    ...
                    {foundUser && (
                        <div className="mt-4 p-4 border border-gray-200 rounded-lg animate-fade-in">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <p className="font-bold text-gray-800">{foundUser.name}</p>
                                    <p className="text-xs text-gray-500">Parent / Guardian</p>
                                    <p className="text-xs text-gray-400">Linked to: {foundUser.student}</p>
                                </div>
                            </div>
                            <button className="btn-primary w-full justify-center bg-red-600 hover:bg-red-700 border-none" onClick={() => alert(`Password reset link sent to ${foundUser.name}`)}>
